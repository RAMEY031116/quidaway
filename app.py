...
st.set_page_config(page_title="QuidAway | Budget Escape Decision Maker", page_icon="🌍", layout="wide", initial_sidebar_state="collapsed")

APP_VERSION = "compact-ads-route-v1"
...
DESTINATIONS = [
    {
        "name": "Porto", "country": "Portugal", "iata": "OPO", "region": "Europe", "budget_band": "££",
        "primary_vibe": "Food", "secondary_vibes": ["City", "Culture", "Relaxed break"],
        "trip_lengths": ["Weekend", "3–5 days"], "best_months": "April–June, September–October",
        "budget_confidence": "Medium", "daily_spend": "£40–£75/day",
        "fake_flight_from": 49, "fake_stay_from": 39, "fake_package_from": 279,
        ...
        "things": ["Ribeira riverside walk", "Dom Luís I Bridge", "Douro Valley day trip", "Francesinha food stop"],
    ...
    {
        "name": "Madeira", "country": "Portugal", "iata": "FNC", "region": "Europe", "budget_band": "££",
        "primary_vibe": "Hiking", "secondary_vibes": ["Nature", "Coastal views", "Warm escape"],
        "trip_lengths": ["3–5 days", "1 week"], "best_months": "March–June, September–November",
        "budget_confidence": "High", "daily_spend": "£45–£80/day",
        "fake_flight_from": 78, "fake_stay_from": 42, "fake_package_from": 389,
    ...
    {
        "name": "Krakow", "country": "Poland", "iata": "KRK", "region": "Europe", "budget_band": "£",
        "primary_vibe": "City", "secondary_vibes": ["Food", "Culture", "Low-cost break"],
        "trip_lengths": ["Weekend", "3–5 days"], "best_months": "April–June, September–December",
        "budget_confidence": "High", "daily_spend": "£30–£60/day",
        "fake_flight_from": 34, "fake_stay_from": 28, "fake_package_from": 219,
    ...
    {
        "name": "Valencia", "country": "Spain", "iata": "VLC", "region": "Europe", "budget_band": "££",
        "primary_vibe": "Beach", "secondary_vibes": ["City", "Food", "Sun"],
        "trip_lengths": ["Weekend", "3–5 days", "1 week"], "best_months": "April–June, September–October",
        "budget_confidence": "Medium", "daily_spend": "£45–£85/day",
        "fake_flight_from": 55, "fake_stay_from": 45, "fake_package_from": 319,
    ...
    {
        "name": "Snowdonia", "country": "Wales, UK", "iata": "MAN", "region": "UK", "budget_band": "£",
        "primary_vibe": "Hiking", "secondary_vibes": ["Nature", "Mountains", "Weekend"],
        "trip_lengths": ["Weekend", "3–5 days"], "best_months": "May–September",
        "budget_confidence": "High", "daily_spend": "£25–£60/day",
        "fake_flight_from": 0, "fake_stay_from": 31, "fake_package_from": 149,
...
html,body,[class*="css"]{font-family:Inter,sans-serif}[data-testid="stAppViewContainer"]{background:var(--bg);color:var(--text)}.block-container{max-width:1160px;padding-top:1rem;padding-bottom:3rem}#MainMenu,footer,header{visibility:hidden}h1,h2,h3,h4,h5,h6,p,li,label,span,div{color:inherit}
...
.qa-route-wrap{margin-top:22px}.qa-route-title{display:flex;align-items:center;gap:10px;margin-bottom:10px}.qa-route-title h3{margin:0;color:var(--text)}.qa-route-card{border-radius:18px;padding:16px;height:100%;position:relative;overflow:hidden}.qa-route-day{display:inline-flex;align-items:center;gap:8px;padding:7px 10px;border-radius:999px;background:var(--chip);color:var(--chiptext);font-weight:950;margin-bottom:12px}.qa-route-item{display:flex;gap:10px;align-items:flex-start;margin:10px 0;color:var(--muted)}.qa-route-time{font-size:.78rem;font-weight:900;color:var(--blue);min-width:70px}.qa-route-text{font-size:.93rem;color:var(--text);font-weight:650}
.qa-panel{border-radius:18px;padding:16px;height:100%}.qa-ad{margin:22px 0;padding:22px;border-style:dashed;border-radius:20px;text-align:center;color:var(--muted);font-weight:850}.qa-side-ad{border:1px dashed var(--border);background:var(--card2);border-radius:18px;padding:18px;text-align:center;color:var(--muted);font-weight:800;margin-top:12px}.qa-footer{text-align:center;color:var(--muted);margin-top:30px;padding-top:18px;border-top:1px solid var(--border);font-size:.9rem}
...
def calc_score(place, region, budget, vibe, trip_length):
    score = 30
    if region == "Surprise me" or place["region"] == region: score += 20
    budget_map = {"£ (Shoestring)": "£", "££ (Moderate)": "££", "£££ (Comfort)": "£££", "Any": place["budget_band"]}
    target = budget_map.get(budget, "££")
    if target == place["budget_band"]: score += 25
    elif target == "£" and place["budget_band"] == "££": score += 10
    elif target == "££" and place["budget_band"] in ["£", "£££"]: score += 10
    if vibe == "Surprise me" or vibe == place["primary_vibe"] or vibe in place["secondary_vibes"]: score += 23
    if trip_length in place["trip_lengths"]: score += 12
    return min(score, 98)
...
def affiliate_url(provider, place, starting_from):
    dest, origin, iata = quote_plus(f"{place['name']} {place['country']}"), quote_plus(starting_from or "London"), quote_plus(place["iata"])
    if provider == "booking": return f"https://www.booking.com/searchresults.html?ss={dest}&aid=quidaway-demo"
    if provider == "skyscanner": return f"https://www.skyscanner.net/transport/flights/{origin}/{iata}/?associateid=quidaway-demo"
    if provider == "expedia": return f"https://www.expedia.co.uk/Hotel-Search?destination={dest}&affcid=quidaway-demo"
    if provider == "getyourguide": return f"https://www.getyourguide.co.uk/s/?q={dest}&partner_id=quidaway-demo"
    return "#"
...
<div class="qa-price-section">
  ...
  <span style="display:inline-flex;padding:7px 11px;border-radius:999px;border:1px solid var(--border);background:var(--card);color:var(--muted);font-size:.8rem;font-weight:850;margin-bottom:12px;">Demo refreshed: {datetime.now().strftime("%d %b %Y")}</span>
    ...
    with c1: price_card("Skyscanner-style flights", flight, f"Compare flights from {starting_from or 'London'} to {place['iata']}.", affiliate_url("skyscanner", place, starting_from), "Compare flights", True)
    with c2: price_card("Booking.com-style stays", stay, f"Search stays in {place['name']}. Demo nightly guide.", affiliate_url("booking", place, starting_from), "Check stays")
    with c3: price_card("Expedia-style packages", package, f"Demo package estimate for {trip_length.lower()} planning.", affiliate_url("expedia", place, starting_from), "View packages")
    c4,c5,c6 = st.columns(3)
    with c4: price_card("GetYourGuide-style activities", "from £18", "Tours, viewpoints and local experiences.", affiliate_url("getyourguide", place, starting_from), "Find activities", True)
    with c5: price_card("Budget confidence", place["budget_confidence"], "How stable this destination feels for planning.", "#", "Why this score?")
    with c6: price_card("QuidAway note", place["budget_band"], "Budget guidance first, live prices before booking.", "#", "Save idea")
...
def route_items(place, days):
    things = place["things"]
    templates = [
        [("Arrival", "Arrive and settle in"), ("Afternoon", things[-1]), ("Evening", "Budget-friendly local dinner")],
        [("Morning", things[0]), ("Afternoon", things[1]), ("Evening", "Viewpoint or local walk")],
        [("Morning", things[2]), ("Lunch", "Local food stop"), ("Evening", "Relaxed final evening")],
  ...
  <div class="qa-route-title">
    ...
    for start in range(0, days, 3):
        ...
        for col, idx in zip(cols, range(start, min(start+3, days))):
            with col:
                items = "".join([f"<div class='qa-route-item'><div class='qa-route-time'>{t}</div><div class='qa-route-text'>{txt}</div></div>" for t,txt in templates[idx]])
...
def render_result(place, score, scored, trip_length, starting_from):
    ...
    with col_text:
        ...
        with m1: mini_card("Budget Band", place["budget_band"])
        with m2: mini_card("Est. Daily", place["daily_spend"])
        with m3: mini_card("Pricing Confidence", place["budget_confidence"], True)
...
<div class="qa-hero">
  ...
  <p>Tell us your budget band, travel vibe and trip length. QuidAway suggests where your money could go furthest, then shows how live partner searches and a route plan could look.</p>
  <div><span class="qa-pill">💸 Budget guidance</span><span class="qa-pill">🌍 Destination decision-maker</span><span class="qa-pill">🔗 Affiliate-ready layout</span><span class="qa-pill">🗺️ Visual route idea</span></div>
</div>""", unsafe_allow_html=True)
...
if submitted:
    ph = st.empty()
    with ph:
        st.markdown("<div class='qa-spinner-box'><div class='qa-spinner'>🌍</div><h3>Searching the globe...</h3><p class='qa-muted'>Matching budget · checking vibes · building route idea</p></div>", unsafe_allow_html=True)
    time.sleep(1.1)
    ph.empty()
...
st.markdown("<div class='qa-ad'>AdSense placeholder — better used on destination/blog sections, or as a small side banner near the image after approval.</div>", unsafe_allow_html=True)

st.markdown("## How this becomes WordPress")
c1,c2,c3 = st.columns(3)
with c1: st.markdown("<div class='qa-panel'><b>1. Static destination data</b>Store airport code, budget band, vibe and route logic in WordPress ACF fields.</div>", unsafe_allow_html=True)
with c2: st.markdown("<div class='qa-panel'><b>2. Dynamic partner links</b>Generate affiliate links/widgets from destination name and IATA code.</div>", unsafe_allow_html=True)
with c3: st.markdown("<div class='qa-panel'><b>3. No exact price promise</b>Show budget guidance and send users to partners for live prices.</div>", unsafe_allow_html=True)
